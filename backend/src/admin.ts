...
export class AdminController {
  ...
  @Get('receipts/:id/url') async receiptUrl(@Param('id') id: string) { const receipt = await this.prisma.comprobante.findUnique({ where: { id } }); if (!receipt) throw new BadRequestException('Comprobante inexistente'); return { url: await this.storage.signedPath(id, receipt.storageKey), expiresIn: 300 }; }
